...
function log () { echo "`hostname` | `date '+%F | %H:%M:%S |'` $1" | tee -a installation.booty.log; }
...
# Important configuration information are gathered here
function gather () {
  echo "[*] ${@}" >> /root/config-info.booty.txt
}
function update_change_log () {
  echo "${CHANGE_ID} | `date "+%F | %H:%M:%S"` | $1 | $2"  >> /root/system-change.booty.log
  ((GLOBAL_CHANGE_ID++))
...
echo "-----------------------------------------------------------------------" >> /root/config-info.booty.txt
echo "READ ME                                                                " >> /root/config-info.booty.txt
echo "Generated by Booty (Version ${VERSION}) Automated installation script    " >> /root/config-info.booty.txt
echo "Date: `date '+%F | %H:%M'`. (c) Akash Mitra [akashmitra@gmail.com]     " >> /root/config-info.booty.txt
echo "-----------------------------------------------------------------------" >> /root/config-info.booty.txt
...
If_Error_Exit "Can not add MariaDB repo to source list"
...
sudo -H -u ${SITEUSER} bash -c '/usr/local/bin/composer install --optimize-autoloader --no-dev --no-interaction'

info "Running artisan commands..."
php ${WEBROOT}/${SITENAME}/${REPONAME}/artisan key:generate
php ${WEBROOT}/${SITENAME}/${REPONAME}/artisan migrate --seed
php ${WEBROOT}/${SITENAME}/${REPONAME}/artisan storage:link
php ${WEBROOT}/${SITENAME}/${REPONAME}/artisan config:cache
